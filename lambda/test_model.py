...
def test_charge_intent_for_today_means_today_before_5am():
    intent = charge_intent(
        100,
        dt.date(2024, 8, 28),
        dt.datetime(2024, 8, 28, 3, 43, 0)
    )
    assert intent == ChargeIntent(100, dt.date(2024, 8, 28))


@pytest.mark.parametrize("future,now,result", [
    (dt.date(2024,8,29), dt.datetime(2024,8,28,17,45,0), dt.date(2024,8,29)),
    (dt.date(2024,8,29), dt.datetime(2024,8,28,3,45,0), dt.date(2024,8,29)),
    (dt.date(2024,8,31), dt.datetime(2024,8,28,12,0,0), dt.date(2024,8,31)),
])
def test_set_intent_for_future_date_sets_that_date(future: dt.date, now: dt.datetime, result: dt.date):
    intent = charge_intent(
        100,
        future,
        now
